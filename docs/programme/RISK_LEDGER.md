...
| R-006 | Malformed font destabilizes Host | Linux metadata parsing runs in a 3-second/1-MB bounded `fc-query` child; malformed/truncated metadata and an actual truncated file are rejected before import. Browser/CoreText rendering is not process-isolated from its normal content/application boundary | Release blocker | Cross-format hostile corpus and process-containment decision |
