...
| Simple Body Copy | One font per 5,152 × 2,160 page, authored/custom copy, full text, one shared fitted reading size | Host validates and transactionally commits `Body Copy/` PNGs | Host validates and transactionally commits `Body Copy/` PNGs | Unit and displayed Mac evidence verified; exact-head Linux candidate evidence and human typography approval pending |
...
| Interface scaling | One 80–140% control across Simple and Studio | Shared renderer | Shared renderer | Mac evidence checks all seven steps plus Body Copy at both extremes, above-44 px minimum at 80%, overflow, and truncation; exact-head Linux candidate run pending |
