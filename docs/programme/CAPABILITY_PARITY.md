...
| Simple intake | Files/folders plus family-first installed picker and style detail feed the same Study | Native panel and CoreText Catalog | Native dialog and Fontconfig Catalog | Displayed Mac and Linux release-source journeys passed; independent-machine acceptance remains pending |
...
| Simple Body Copy | One font per 5,152 × 2,160 page, authored/custom copy, full text, one shared fitted reading size | Host validates and transactionally commits `Body Copy/` PNGs | Host validates and transactionally commits `Body Copy/` PNGs | Unit, displayed Mac, and exact-head Linux release evidence passed; human typography approval pending |
...
| Interface scaling | One 80–140% control across Simple and Studio | Shared renderer | Shared renderer | Mac release evidence checks all seven steps plus Body Copy at both extremes, above-44 px minimum at 80%, overflow, and truncation; Linux release-source displayed and package journeys also passed |
