            ...
            return (
              <article className={`specimen-card ${candidate.id === selected.id ? "is-selected" : ""}`} key={candidate.id}>
                ...
                <div className="card-actions" role="group" aria-label={`Review ${candidateFace.family} ${candidate.label}`}>
                  {REVIEW_STATES.map((reviewState) => <button type="button" key={reviewState} className={candidate.reviewState === reviewState ? "is-active" : ""} aria-label={reviewLabels[reviewState]} aria-pressed={candidate.reviewState === reviewState} onClick={() => dispatch({ type: "set-review-state", candidateIds: [candidate.id], reviewState })}>{reviewGlyphs[reviewState]}</button>)}
                </div>
              </article>
            );
          })}
          ...
          {candidates.map((candidate, index) => {
            ...
            return (
              <article className="compare-card" key={candidate.id}>
                ...
                <div className="compare-footer"><span>{Math.round(size)} px · {policyLabels[policy].label}</span><span className="reorder-buttons"><button type="button" aria-label="Move left" onClick={() => move(index, -1)} disabled={index === 0}>←</button><button type="button" aria-label="Move right" onClick={() => move(index, 1)} disabled={index === candidates.length - 1}>→</button><button type="button" aria-label="Remove from comparison" onClick={() => dispatch({ type: "toggle-tray", candidateId: candidate.id })}>Remove</button></span></div>
              </article>
            );
          })}
        </div>
