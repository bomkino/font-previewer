...
private final class MacEvidenceRunner {
    ...
    private func run() async throws {
        ...
        let beforeTerminationCallback = try await inspect(); let terminationsBefore = host.processTerminations; host.webViewWebContentProcessDidTerminate(host.webView); try await wait("simulated WebKit termination callback recovery") { try await self.bool("document.querySelector('#workspace-heading') && document.querySelector('.candidate-row[aria-current=\"true\"] .review-glyph')?.getAttribute('aria-label')==='Keep'") }; let afterTerminationCallback = try await inspect(); trace["terminationCallbackRecovery"] = ["simulatedDelegateCallback": true, "counterAdvanced": host.processTerminations == terminationsBefore + 1, "before": beforeTerminationCallback, "after": afterTerminationCallback]
        let before = try await inspect(); host.webView.reload(); try await wait("Reload") { try await self.bool("document.querySelector('#workspace-heading') && document.querySelector('.candidate-row[aria-current=\"true\"] .review-glyph')?.getAttribute('aria-label')==='Keep'") }; let after = try await inspect(); trace["reload"] = ["before": before, "after": after]
