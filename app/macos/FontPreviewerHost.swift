...
private final class MacEvidenceRunner {
    ...
    private func run() async throws {
        ...
        let checks: [(String, Bool)] = [
            ...
            ("semantics", semantics?["unnamed"] as? Int == 0
                ...
                && ((semanticLayout?["minControlHeight"] as? Double) ?? 0) >= 44
                && ((semanticLayout?["iconOnlyButtons"] as? Int) ?? 0) > 0
                && semanticLayout?["iconCentersAligned"] as? Bool == true
                && ((semanticLayout?["selectCarets"] as? Int) ?? 0) >= 3
                && semanticLayout?["selectCaretsComplete"] as? Bool == true
                && semanticLayout?["selectCaretsCentered"] as? Bool == true
                && ((semanticLayout?["selectCaretInset"] as? Double) ?? 0) >= 14
                && ((semanticLayout?["panelTopSpread"] as? Double) ?? .infinity) <= 1
                && ((semanticLayout?["panelBottomSpread"] as? Double) ?? .infinity) <= 1
                && ((semanticLayout?["panelTrayGap"] as? Double) ?? .infinity) <= 1
                && ((semanticLayout?["horizontalOverflows"] as? [Any]) ?? [NSNull()]).isEmpty),
            ("disclosure motion", disclosure?["expanded"] as? Bool == true
                && disclosure?["collapsed"] as? Bool == true
                && disclosure?["openIntermediate"] as? Bool == true
                && disclosure?["closeIntermediate"] as? Bool == true
                && disclosure?["caretRotates"] as? Bool == true),
    ...
    private func semanticAudit() async throws -> [String: Any] {
        try await host.evaluate(#"(()=>{const controls=[...document.querySelectorAll('button,input,select,textarea')];const named=el=>el.getAttribute('aria-label')||el.getAttribute('aria-labelledby')||el.closest('label')?.textContent?.trim()||el.textContent?.trim();const roleLabel=[...document.querySelectorAll('.field-label')].find(label=>label.querySelector(':scope > span')?.textContent?.trim()==='Role');const roleSelect=roleLabel?.querySelector('select')?.getBoundingClientRect();const roleHelp=roleLabel?.querySelector(':scope > small')?.getBoundingClientRect();const visible=el=>{const rect=el.getBoundingClientRect(),style=getComputedStyle(el);return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden'};const measured=controls.filter(el=>visible(el)&&!(el instanceof HTMLInputElement&&['checkbox','radio','range'].includes(el.type)));const iconButtons=controls.filter(el=>el instanceof HTMLButtonElement&&visible(el)&&el.querySelector(':scope > .interface-icon')&&!el.textContent?.trim());const iconDeltas=iconButtons.map(button=>{const icon=button.querySelector(':scope > .interface-icon'),a=button.getBoundingClientRect(),b=icon.getBoundingClientRect();return {x:Math.abs(a.left+a.width/2-b.left-b.width/2),y:Math.abs(a.top+a.height/2-b.top-b.height/2)}});const caretDeltas=[...document.querySelectorAll('.select-control')].filter(visible).map(shell=>{const select=shell.querySelector('select'),caret=shell.querySelector(':scope > .interface-icon-caret-down'),a=select?.getBoundingClientRect(),b=caret?.getBoundingClientRect();return {complete:Boolean(a&&b),y:a&&b?Math.abs(a.top+a.height/2-b.top-b.height/2):999,trailingSpace:a&&b?a.right-b.right:0}});const panels=['.catalog','.workspace','.inspector'].map(selector=>document.querySelector(selector)?.getBoundingClientRect()).filter(Boolean),tray=document.querySelector('.tray')?.getBoundingClientRect();const topSpread=panels.length?Math.max(...panels.map(rect=>rect.top))-Math.min(...panels.map(rect=>rect.top)):999,bottomSpread=panels.length?Math.max(...panels.map(rect=>rect.bottom))-Math.min(...panels.map(rect=>rect.bottom)):999,trayGap=tray&&panels.length?Math.max(...panels.map(rect=>Math.abs(rect.bottom-tray.top))):999;return {controls:controls.length,unnamed:controls.filter(el=>!named(el)).length,mains:document.querySelectorAll('main').length,asides:document.querySelectorAll('aside').length,duplicateIds:[...document.querySelectorAll('[id]')].map(el=>el.id).filter((id,i,a)=>a.indexOf(id)!==i).length,horizontalOverflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,roleHelpSeparated:Boolean(roleSelect&&roleHelp&&roleSelect.bottom<=roleHelp.top),layout:{minControlHeight:measured.length?Math.min(...measured.map(el=>el.getBoundingClientRect().height)):0,iconOnlyButtons:iconDeltas.length,iconCentersAligned:iconDeltas.every(delta=>delta.x<=1&&delta.y<=1),selectCarets:caretDeltas.length,selectCaretsComplete:caretDeltas.length>0&&caretDeltas.every(caret=>caret.complete),selectCaretsCentered:caretDeltas.length>0&&caretDeltas.every(caret=>caret.y<=1),selectCaretInset:caretDeltas.length?Math.min(...caretDeltas.map(caret=>caret.trailingSpace)):0,panelTopSpread:topSpread,panelBottomSpread:bottomSpread,panelTrayGap:trayGap,horizontalOverflows:[...document.querySelectorAll('.catalog,.workspace,.inspector')].filter(visible).filter(el=>el.scrollWidth>el.clientWidth+1).map(el=>el.className)}}})()"#) as? [String: Any] ?? [:]
    }
    private func disclosureMotionAudit() async throws -> [String: Any] {
        try await host.evaluateAsync(#"(async()=>{const disclosure=document.querySelector('.inspector-details'),trigger=disclosure?.querySelector('.inspector-disclosure-trigger'),content=disclosure?.querySelector('.inspector-details-content'),caret=trigger?.querySelector('.interface-icon-caret-right');if(!(disclosure instanceof HTMLElement)||!(trigger instanceof HTMLButtonElement)||!(content instanceof HTMLElement)||!(caret instanceof SVGElement))throw new Error('Missing disclosure motion surface');const reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches,frames=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))),wait=ms=>new Promise(resolve=>setTimeout(resolve,ms)),measure=()=>({height:disclosure.getBoundingClientRect().height,expanded:trigger.getAttribute('aria-expanded')==='true',inert:content.inert,hidden:content.getAttribute('aria-hidden')==='true',caret:getComputedStyle(caret).transform});const collapsed=measure();trigger.click();await frames();await wait(60);const opening=measure();await wait(180);const expanded=measure();trigger.click();await frames();await wait(60);const closing=measure();await wait(180);const closed=measure();return {reducedMotion,expanded:expanded.height>collapsed.height+20&&expanded.expanded&&!expanded.inert&&!expanded.hidden,collapsed:Math.abs(closed.height-collapsed.height)<=1&&!closed.expanded&&closed.inert&&closed.hidden,openIntermediate:reducedMotion||(opening.height>collapsed.height+1&&opening.height<expanded.height-1),closeIntermediate:reducedMotion||(closing.height<expanded.height-1&&closing.height>closed.height+1),caretRotates:expanded.caret!==collapsed.caret&&closed.caret===collapsed.caret,transitionDuration:getComputedStyle(content).transitionDuration,samples:{collapsed,opening,expanded,closing,closed}}})()"#) as? [String: Any] ?? [:]
    }
